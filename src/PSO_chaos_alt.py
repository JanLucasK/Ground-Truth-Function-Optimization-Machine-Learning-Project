...
class PSO_optimizer():
    ...
    def optimize(self, initial_guess, niter=100):
        ...
        result_nn, _ = pso(self.call_nn, swarmsize=niter, maxiter=niter, minstep=1e-5, lb= [-5,-5], ub=[5, 5])
        result_bbob, _ = pso(self.call_bbob, swarmsize=niter, maxiter=niter, minstep=1e-5, lb= [-5,-5], ub=[5, 5])
    ...
    # Pass the grid data through the model to get predictions
        ...
        for ax, model_values, path in zip(axes, [predictions, function_values], [path_model, path_bbob]):
            ...
            ax.scatter(x=result_bbob[0], y=result_bbob[1], c='black', s=250, marker='x')  # Add this line for PSO
...
model_path = 'models/training_v1_f01_2.pth'
