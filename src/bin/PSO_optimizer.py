...
from pyswarm import pso  # Import PSO from the pyswarm library
...
class PSO_optimizer():
    ...
    # Convert x to torch tensor
        ...
        model_point = [x[0], x[1], y_scalar]  # This is the point to be appended to the path
        self.model_path.loc[len(self.model_path)] = model_point
        self.path.append([x[0], x[1]])  # Append the [x, y] coordinates to the path

        return y_scalar

    
    def optimize(self, initial_guess, niter=100, swarmsize=100):
        # Use PSO optimizer from pyswarm library
        result_nn, _ = pso(self.call_nn, lb=[-5, -5], ub=[5, 5], maxiter=niter, swarmsize=swarmsize, args=())
    

        
        # Perform the same steps as before for visualization and comparison
        model_path = np.array(self.path)
        self.path = []
        result_bbob = self.call_bbob(result_nn)  # Call bbob function for the PSO result
    ...
    # Pass the grid data through the model to get predictions
        ...
        # Plot particle trajectories
            for traj in path:
                traj = np.array(traj)
                ax.plot(traj[:, 0], traj[:, 1], marker='', linewidth=1, color='white', alpha=0.5)
        
        # Plot optimal point
            ax.scatter(x=result[0], y=result[1], c='black', s=250, marker='x')
        plt.show(block=True)
        return plt
...
# call Bbob for minimization
    ...
    # Create an instance of the PSO_optimizer class and call the optimize method
if __name__ == "__main__":
    model_path = "models/training_v1_f24_3.pth"
    input_bounds = [(-5, 5), (-5, 5)]  # Example input bounds
    
    pso_optimizer = PSO_optimizer(model_path, input_bounds)
    initial_guess = [0, 0]  # Example initial guess
    niter = 100
    swarmsize = 100
    result_nn, result_bbob, fig = pso_optimizer.optimize(initial_guess, niter, swarmsize)
